...
class YoloDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic:
            if self.rand() < 0.5:
                ...
                image, box, img_shape = self.get_random_data(self.annotation_lines[index], self.input_shape,
                                                             random=self.train)
        else:
            image, box, img_shape = self.get_random_data(self.annotation_lines[index], self.input_shape,
                                                         random=self.train)
        ...
        return image, box, img_shape, self.annotation_lines[index]
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=1.5, val=1.5, random=True):
        ...
        return image_data, box, (nh if nh < h else h, nw if nw < w else w)
...
def yolo_dataset_collate(batch):
    ...
    img_shapes = []
    annotation_lines = []
    for img, box, img_shape, annotation_line in batch:
        images.append(img)
        bboxes.append(box)
        img_shapes.append(img_shape)
        annotation_lines.append(annotation_line)
    images = np.array(images)
    return images, bboxes, img_shapes, annotation_lines
